    ...
    if (thread) {
      api.getPosts(thread).then((fetchedPosts) => {
        ...
        const sortedPosts = [...fetchedPosts].sort(
          (a, b) => parseInt(a.post_time) - parseInt(b.post_time),
        );
          ...
          <TableBody>
            {posts?.map((post: any) => (
              ...
              >
                ...
                <TableCell className="overflow-scroll">
                  <div
                    className="break-words"
                    dangerouslySetInnerHTML={{
                      __html: truncate(post.post_content_html, { length: 100 }),
                    }}
                  ></div>
                </TableCell>
              </TableRow>
...
const PostModal: React.FC<PostModalProps> = ({ show, onHide, post }) => {
  return (
    <Dialog open={show} onOpenChange={(open) => !open && onHide()}>
      <DialogContent>
        ...
        <div
          className="w-full max-h-[60vh] overflow-scroll"
          dangerouslySetInnerHTML={{
            __html: post?.post_content_html ?? "<div></div>",
          }}
        ></div>
