...
const Threads: React.FC<ThreadsProps> = ({ api }) => {
  ...
  useEffect(() => {
    ...
    if(forum)
      api.getThreads(forum).then(setThreads)
  }, [q])
        ...
        <Table>
          ...
          <TableBody>
            {threads?.map((thread: any) => (
              <TableRow key={thread.thread_id} className="cursor-pointer" onClick={() => {
                ...
                <TableCell>{thread.thread_status === 'locked' ? 'Locked' : 'Open'}</TableCell>
                <TableCell>{new Date(parseInt(thread.thread_lastpost_time) * 1000).toLocaleString()}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
